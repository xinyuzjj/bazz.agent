...
| query-vip-loan-fixed-rate-market | `loan-coin` [`duration` `current` `size`] | Query VIP Loan Fixed Rate Market |
...
| vip-loan-fixed-rate-borrow | `supply-request` `borrow-coin` `loan-term` `borrow-uid` `collateral-coin` `collateral-account-id` [`auto-repay`] | VIP Loan Fixed Rate Borrow |
...
| get-vip-loan-repayment-history | [`order-id` `loan-coin` `start-time` `end-time` `current` `limit`] | Get VIP Loan Repayment History |
