...
| create-sub-account-api-key | `email` `api-name` `status` [`can-trade` `can-margin-loan-repay` `can-futures-trade` `can-universal-transfer` `can-vanilla-options` `ip-address` `third-party-name` `public-key`] | Create Sub-account API Key (For Master Account) |
...
| delete-sub-account-api-key | `email` `sub-account-api-key` [] | Delete Sub-account API Key (For Master Account) |
...
| modify-sub-account-api-key-permission | `email` `sub-account-api-key` [`can-trade` `can-margin-loan-repay` `can-futures-trade` `can-universal-transfer` `can-vanilla-options`] | Modify Sub-account API Key Permission (For Master Account) |
| query-sub-account-api-key | `email` [`sub-account-api-key` `page` `size`] | Query Sub-account API Key (For Master Account) |
...
| get-sub-account-deposit-history | `email` [`include-source` `coin` `status` `start-time` `end-time` `limit` `offset`  `tx-id`] | Get Sub-account Deposit History (For Master Account) |
...
| query-sub-account-assets-asset-management | `email` [] | Query Sub-account Assets V4 (For Master Account) |
...
**from-account-type:** `SPOT` `USDT_FUTURE` `COIN_FUTURE` `MARGIN` `ISOLATED_MARGIN`
**product-type:** `UM`
**to-account-type:** `SPOT` `USDT_FUTURE` `COIN_FUTURE` `MARGIN` `ISOLATED_MARGIN`
