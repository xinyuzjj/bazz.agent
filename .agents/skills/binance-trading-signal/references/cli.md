# binance-trading-signal — CLI Reference
...
| `exitRate` | number | Smart money exit rate (0-100 integer; high = stale, ≥ 70 = exiting) |
| `status` | string | Signal status. Values: `valid` (fresh/open), `timeout` (expired/stale), `outDecline` (price declining), `exitRate` (exit threshold reached), `null` (no status). |
...
**Signal quality:** `smartMoneyCount ≥ 5` = stronger conviction · `exitRate ≥ 70` = smart money exiting, treat with caution · `status:"timeout"` = no longer actionable.
