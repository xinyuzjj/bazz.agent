...
- **能力（LLM function-calling，9 工具）**：`scan_market` 扫描异常 · `check_risk` 风险教育 · `propose_trade` 生成带止损止盈方案(需确认才下单) · `market_quote` 查价 · `explain_x402` · `list_skills` · `onchain_ops` · `memory_write` 记长期偏好 · `get_help` 列能力。
...
### C5. 账户持仓/盈亏看板 + 风险熔断（靠 C2 client 长出）
...
## E. 安全红线（设计/实现都别破）
- Exchange API Key/Secret **绝不明文存**(现 LLM key 明文存 SQLite，须一并改加密)。
- 下单/平仓/撤单一律二次确认。
