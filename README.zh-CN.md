...
| 产物 | 怎么跑 |
|------|--------|
| 🎒 [portable.zip](https://github.com/xinyuzjj/bazz.agent/releases/latest) | 解压到任意目录 → 双击 `BAZZ.AGENT.exe` |
| 📦 [setup.exe](https://github.com/xinyuzjj/bazz.agent/releases/latest) | 安装到 `%LOCALAPPDATA%`，带开始菜单 / 桌面快捷方式 / 卸载器 |

- **无需 Python、无需 Node、无需安装**——Electron + PyInstaller 熔成一个应用。
- 首次启动 ~3–5 秒（后端预热），随后舱门打开。
- 数据在 `<安装目录>\workspace`（安装盘只读时自动兜底到 `%APPDATA%\BAZZ.AGENT\workspace`）——便携版整个文件夹随便搬，数据跟着走。
- 无边框窗口：按住顶栏拖动，右上 `─ / □ / ✕` 控制。
