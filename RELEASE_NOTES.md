...
## 🆕 v1.7.2 更新要点（链上筹码接线：链下代理指纹 × 链上持仓证据，交叉打假）

**这一版把一个躺了两版、从没被 import 过的模块接进了生产链路。** `src/onchain.py` 早就写好了
（989 行 / 47 KB），但**全仓没有任何一处 import 它** —— 一个没有调用方的模块，等于没写。本轮把
它接进妖币雷达，接的过程中**用真实链路打出两个真 bug**（都不是逻辑错，都是「静默」型：

### ① 先纠正一个前提：这个模块此前**一行代码都没跑过**
`grep -rn "import onchain" src/ desktop_app.py` 命中 **0 次**。它是「写完就搁那儿」的状态 ——
没测试、没接线、没发版。所以本轮的顺序是**先跑真实链路再谈接线**：不跑就不知道它是能用的还是坏的，
而「读代码看着没问题」正是这类模块最常见的假象。

### ② 真 bug 一：代理端口被写死成 7899，而这个端口是**动态分配**的
`onchain.py` 里把「内核混合端口」硬编码成 `http://127.0.0.1:7899`。但 `proxy_kernel` 的混合端口
是 `_free_port(7899)` **动态挑**的 —— 7899 被占（实测经常被占）时会换一个端口。

| 情况 | 硬编码 7899 的后果 |
|---|---|
| 7899 恰好是本项目内核 | 侥幸能用 |
| 7899 被别的进程占 | 请求指向**别人的端口** |
| 内核在别的端口 | 整层**静默全空**（不报错、只是「没有链上数据」） |

**修法**：删掉硬编码，改成运行期向内核问端口 ——
...
护栏钉的是「源码里不得出现字面量 `127.0.0.1:7899`」—— **注意是字面量，源码扫描走 `tokenize`
剥注释与 docstring**，否则这条断言会被它自己的修复说明绊倒（本项目已经栽过两次）。

### ③ 真 bug 二：GoPlus 限流**不是 429，是 HTTP 200 里藏了个 code**
这条是本轮最有价值的实测。GoPlus 被限流时返回的是：

```
HTTP 200  {"code": 4029, "message": "too many requests"}      ← 没有 result 键
```

而原来的解析逻辑是「取 `result`，没有就当作这个币没数据」→ **限流被读成了「没测到」**。
两者在界面上长得一模一样，但含义完全相反：一个是数据源在拒我，一个是我真没查到。
于是「链上筹码」这一块会**静默地时好时坏**，而且永远不报错。
...
| 限流后等 30s 重试 | 仍在限流 |
| 限流后等 **45s** 重试 | **恢复** |
| **统一 5.5s 间隔** | **14/14 全成功（82 秒）** |

结论：限流按**窗口内次数**算，慢下来就能绕开，不需要靠退避硬扛。所以实现是三件事一起做 ——
**`_throttle_signal()`**（识别 `code in ("4029","429")` 或消息含 `too many requests`，
**必须在解析 `result` 之前判**，否则限流又变成「没数据」）+ **`_src_available()` 短路**
（退避窗口内直接不发请求）+ **逐源计数**（`goplus_throttled` / `rug_throttled` 进 `_stats`，
「被限了几次」从此是可见的数字，不是猜的）。

### ④ 交叉判据：链下代理指纹 × 链上持仓证据，**只给证据不改分级**
新函数 `cross_verdict(reading, struct_flags)` 六态：

| 状态 | 触发条件 | 含义 |
|---|---|---|
| `confirm` | 链下命中结构性指纹 **且** 链上 top10 高 | 两侧互证 |
| `refute` | 链下命中 **但** 链上分散 | **链下代理可能是误报**（最有价值的一档） |
| `warn` | 链下未命中 **但** 链上高度集中 | 链上先出现异常 |
| `clean` | 两侧都干净 | — |
| `neutral` | top10 落在 25%~60% | **不判**（不编故事） |
| `unknown` | 这个币没有读数 | 是**这个币**的属性 |

**阈值 60 / 25 不是拍的**，是拿真实 16 币的 `top10_pct` 分布定的（**中位 45.5**）：
取 60 让报警保持稀有，取 25 让「干净」不被随便给。

⚠️ **结构性指纹只有 3 个能参与交叉**：`无现货 / 合约独大 / 换手畸高`。另外两个
（`空头付钱` / `拉升无爆仓`）是**持仓侧**指纹 —— 拿它和「链上持仓集中度」比大小是
**两个范畴的东西互比**，属于凭空造因果。这条落成模块级常量并在测试里钉死两侧名单不得漂移。

⚠️ **链上证据不参与 `grade` 赋值**。这是刻意的：新数据在没有样本之前**不许改判据**。
它只往 `evidence` 里追加一行可读证据（`chain` / `chain_struct` 两个字段供消费方取用）。

### ⑤ 「先剔再算」：烧地址会把结论整个反过来
`top10_pct` 原始口径把**烧掉/销毁地址**算进去，而这些地址的筹码**永远不会卖**。
实测 CAKE（BSC）：

| 口径 | top10 占比 | 判据结论 |
|---|---|---|
| 原始（含烧地址） | **92.74%** | 极度集中 → `confirm` |
| **剔除烧地址后** | **4.14%** | 极度分散 → **`refute`** |

**结论完全反过来。** 所以取值路径一律「**先剔再算**」，被剔掉的个数作为审计字段带出来。

### ⑥ 三个数据源（零 Key、全免费）
| 源 | 覆盖 | 直连 | 说明 |
|---|---|---|---|
| CoinGecko | L1/L2 层，symbol → cg_id → 合约地址 | ❌ 必须走代理 | 只做地址解析 |
| GoPlus | EVM + Solana 持仓 / 貔貅 / 税率 | ✅ | 5.5s 间隔 |
| RugCheck | Solana 前 20 持有者 + 风险项 | ✅ | — |

### ⑦ 「整条链路不通」与「这个币没有链上数据」必须分开说
`_oc_cell(reading, manip_flags, measured)` 在 `measured=False` 时出 **`source_down`**，
与逐币的 `unknown` **是两个不同的 state 码**。理由：前者是**我们的问题**（代理没通 / 源挂了），
后者是**这个币的属性**。把它们合并，就等于替数据源背锅 —— 用户看到「没有链上数据」会以为
是币的问题，实际是我们没打通。payload 另外下发 `onchain_health()` 快照（含
`goplus_available` / `goplus_backoff_sec` / `rug_backoff_sec` / `route` / `stats`），
前端体检条在未通/限流时**红标**，正常时显示「链上 N 币」。

### ⑧ 派发必须**在逐币循环之前**
链上读数是**异步刷新 + 读缓存**（绝不阻塞雷达主路径）。所以 `_onchain_dispatch(pool_syms)`
放在扫描主体**开头**、`_onchain_read(pool_syms)` 在循环前**读一次**（不是每行读一次）。
派发晚一步，这一轮末尾就永远填不上读数 —— 表现是「链上那一列一直是空的」，不报错。

### ⑨ 前端：state 码 → i18n 键走 **switch**，不走动态键
`t(\`markets.onchain.${state}\`)` 这种动态键会**绕开「用到的键必须两本字典都有」的静态扫描** ——
而且 `i18n.tsx` 的实现是 `dict[key] ?? key`（**没有 zh 兜底**），缺键就是界面把 key 当文案印出来。
所以改成显式 `switch` 映射到 16 个具名键（zh + en 各一次，共 32 处）。另外链上 pill 渲染的是
**i18n 标签**而不是后端下发的中文 `label` —— 否则英文界面串中文。

### ⑩ 护栏与变异验证
- 新增 `tests/test_v172_onchain.py` —— **181 条断言**，A~K 共 11 组：符号归一化、单位归一化
  （GoPlus 小数 ×100，RugCheck 百分比**原样**，两个源单位相反）、烧地址剔除（CAKE 必须翻转结论）、
  限流识别+计数+退避+短路、动态端口、损坏缓存不连坐、六态交叉判据含边界、只有结构性指纹能参与、
  扫描接线 + `source_down` ≠ `unknown`、monster 轴「只给证据」、前端契约（switch 覆盖全部 state 码 +
  不得用动态键 + i18n 成对）、模块真的被 import + 探针脚本已清理。
- 新增 `tests/_mutate_v172.py` —— **26 组变异，全部转红（26/26）**。第一轮有 1 组「锚点命中 0 次」
  被跳过（锚点与源码换行不一致，等于这条护栏**根本没被测到**），改正锚点后复跑转红。
  这正是变异验证的用处：**它测的不是代码，是护栏本身**。
- `tests/_e2e_v172_onchain.py` —— 真实链路：代理自动选择、后台刷新不阻塞、真实读数与分布、
  CAKE 烧地址翻转、GoPlus 限流记账、逐币 `row_cell`；`--radar` 选项跑真实 `scanner.get_radar_v2`
  并核对 payload `onchain` 与行级 `onchain` 格。
- **修 2 个真 bug 的方式是「先跑真实链路」，不是读代码** —— 这两条（硬编码端口 / 限流读成没数据）
  跑离线测试**永远发现不了**，因为它们在测试里不会复现。

### ⑪ 清理
删掉 4 个临时探针脚本（`_probe_onchain.py` / `_probe_diag.py` / `_probe_syms.py` / `_probe_wif.py`）
与产物目录 `_ws_onchain/` —— 它们的作用已经固化成 `tests/_e2e_v172_onchain.py`，探针留在仓里只会腐烂。

---

## 🆕 v1.7.1 更新要点（妖币雷达问题清单**整波落地** · 收尾：延迟可见 + 成本模型两处「系统性偏乐观」）

**这一版把分析报告里的问题清单全部处理完**，它由两部分组成：**上一版之后攒下的整波**
「按报告逐项落地」（内部编号 v1.6.6，见 ⑥）与**本轮收尾**（延迟可见 + 成本模型两处
「系统性偏乐观」，见 ①~⑤）。
收尾这两处都不会让任何测试变红 —— 一个只是让界面显得比实际新鲜，另一个只是让成本被算少。
这类问题只能靠**实测**发现，所以下面每条都附了实测数据。

### ① C3 端到端延迟：先实测，再动手 —— 结果推翻了原来的判断
报告原先把延迟链上的三个固定项并列（触发层 15m K 线 / 后端 5 分钟缓存 / 前端 180s 轮询），
并把「15m K 线」当成主要嫌疑。**实测澄清它不是瓶颈**：币安 K 线接口返回的最后一根是
**「在途未收盘」K 线**（实测它的收盘时间比当前时间**晚 713 秒**），而触发层特征直接读这最后一根 ——
**每轮扫描用的都是当下最新价，不存在「等 15 分钟收盘」的盲区**。真正的固定项是后两者。于是：

- **后端缓存时长可配**：新增 `BAZZ_RADAR2_TTL` 环境变量（**默认仍是 5 分钟，行为不变**），
  让「少打接口」与「更快看到」之间可以按需取舍。顺手挡掉 `inf` / `nan` —— 放过去会让缓存
  **永不失效**，表现为「雷达再也不更新」。
- **延迟变成看得见的数字**：后端在所有返回路径上补发 `age_sec`（这份数据算于几秒前）与
  `stale`（是否已超过本轮生效的缓存时长）。此前前端只能拿**本地时钟**去减，于是三个问题同时存在：
  各页面各算各的、拿不到后端认为的缓存时长、时钟有偏差时直接算错且方向不固定。
- **前端轮询 180s → 60s**，雷达面板上多一个新鲜度小标：正常时显示「数据 47s 前」，
  **陈旧时变红**并写明「已超过本轮缓存时长」—— 以前「拿到的是上一轮旧缓存」和「刚算完」
  在界面上长得一模一样。
- 这一步**不会**多打币安接口：后端 5 分钟缓存内命中，轮询变密只是多几次本地请求。

### ② 成本模型（1/2）：资金费率结算周期**不是全市场 8 小时**
成本模型把资金费率结算周期**写死成 8 小时**。实测 `/fapi/v1/fundingInfo`（782 条，覆盖
成交额前 110 全部）的周期分布是：
...
**4 小时才是多数**；成交额前 110 里有 **40 个**是 4 小时（ENA / HYPE / ONDO / TRUMP / PENGU /
TAO / PUMP / ZEC / XAU…）。也就是说，对这些币的资金费率成本**被低估了 2 倍**，1 小时周期的
**低估 8 倍**。真实扫描里 90 个币有 **52 个**是 4 小时周期 —— 不是边角情况，是多数情况。

- `sim_cost()` 新增第三个参数（**默认值让旧的两参数调用结果逐字节不变**），周期逐币下发；
  追踪侧**零外呼**读扫描缓存，不在纯计算路径里发网络请求。
- **三态严格区分**：具体小时数 = 实测；`8` = 接口通了但该币用币安默认周期；
  `null` = **没测到** → 回退默认 8 小时，但消费方**自知那是假设**。这个区分是必须的 ——
  把后两者合并，就等于拿「假设的 8h」冒充「实测的 8h」，正是这次要修的同一个毛病。
- 复盘的 Markdown 里写明用了哪个结算周期，数字口径可追溯。

### ③ 成本模型（2/2）：滑点「等样本攒够再标定」的闭环**当时是断的**
代码注释一直写着「滑点 0.15% 是保守常数、不是实测值，等时序库攒够『价差 + 成交额』样本
再回来标定」。但时序库里那列价差存的是**现货**行情的买卖价，而**纯合约币恒为空** ——
偏偏妖币里「只上合约、没有现货深度」正是最经典的形态，于是**最该标定的那批币一个样本都不会有**。

- 修法不是逐币拉订单簿（110 个币 = 110 次请求），而是用 `/fapi/v1/ticker/bookTicker`
  **不带 symbol**：一次请求返回**全市场 766 个**合约的买一卖一（实测 0.2~1.6 秒）。
- 时序库新增 `fut_spread_bps` 列（**与现货口径并列，不覆盖**），老库自动补列。
- 实测全市场价差**中位 3.97~4.2 bps / p90 9.8~10.2 bps / 最大 29.1 bps** —— 都**小于**假设的
  15 bps/边，说明那个常数方向**偏保守**（高估成本，不会把结论说得好听）。
- ⚠️ **但不要因此把常数改成「用盘口价差」**：盘口买一卖一是**下限**，而成本模型估的是
  **市价单实际成交均价** —— 薄盘里吃穿几档的冲击远大于顶档价差。样本的用途是「验证 15bps
  够不够保守」，不是「替换成 4bps」。代码里加了 `SLIP_SRC = "constant"` 显式标出这一点。

### ④ 未调用端点：复核后从「9 个」修正为「6 个」，逐条给出裁决（接 2 弃 4）
原先的清单口径偏宽，把注释与已否掉的兜底路径也算进去了。真正一次都没调用的是 6 个：

- **接入 2 个**：`fundingInfo`（修掉上面那个 2 倍低估）；`depth` 的需求由 `bookTicker`
  以 **1/110 的请求量**满足。
- **明确不接 4 个，理由逐条写明**：`markPriceKlines` / `indexPriceKlines` 能给的
  「标记价对指数价偏离序列」已由时序库的基差列在攒（零额外请求）；`continuousKlines`
  对永续是冗余（合约 K 线接口返回的就是该永续自己的 K 线）；`aggTrades` 的正确形态是
  **WebSocket 流**而不是轮询，属**新增信号**而非本次要补的缺口。
- 顺带核过两条并记下坑：`/futures/data/basis` 与现有基差**是同一个量**（冗余），
  且它的年化字段会返回**空字符串**（直接转浮点会崩）；`indexPriceKlines` **必须用 `pair=`**，
  用 `symbol=` 返回 400。

### ⑤ 验证
- 新增 `tests/test_v171_latency_cost.py`（**109 条断言**）+ 变异验证脚本
  `tests/_mutate_v171.py`（**37 组变异，全部转红**）+ 真实链路脚本 `tests/_e2e_v171_sources.py`。
- **变异验证第一轮有 5 组没红，逐条查出是「护栏写弱了」**（用 `>=` 计数、在整文件里搜样式类名、
  数某个字段名出现次数导致三处混为一谈、锚点命中 2 次），收紧后复跑全红 ——
  这正是变异验证的用处：**它测的不是代码，是护栏本身**。
- 全量 **27 个套件通过**；前端 `tsc --noEmit` 退出码 0、`vite build` 成功；
  **真实扫描端到端通过**（90 个币 / 90 个有结算周期 / 86 个有合约价差，缺的 4 个是纯现货币属正常；
  `age_sec=0` `stale=false`；时序库 107 行 × 16 列）。
- 顺手修掉上一版留在报告里的 2 处 Markdown 粗体泄漏（`**…**` 没转成 HTML）与 1 个游离 `</b>`。

### ⑥ 同批上线：妖币雷达「问题清单」整波落地（内部编号 v1.6.6）

**这一波不在上一版里，而是与 v1.7.1 同批上线** —— 上一版（v1.5.67）之后攒下的一整轮
「按分析报告逐项落地」的工作。报告 `outputs/monster-radar-analysis.html` 的 §8 十三项 +
§15 C1/C2/C4 + §17 档一七项 + 档二三项 + 本轮新发现 D1 **全部落成真实代码**，
并把档三「免费数据能力实测」跑完。挑几条对使用影响最大的：

- **日报 24h 涨幅恒显示 `+0.00%`**：两个引擎都不产出 `change_pct` 这个字段名，取到的永远是空。
  改为按引擎取 `change24_pct`、v1 回退 `change7d_pct`、都没有就**如实写 `24h n/a`**。
- **前端 taker 摘要阈值仍是旧的 1.5**（后端 v1.5.66 已改 1.30）：等于把刚修好的信号又挡回去。
  改为**由 payload 单一来源下发**（新增 `thresholds` 12 个常量），前端读下发值，兜底常量与后端同值。
- **战绩总胜率被 3 笔历史空单污染**：`radar_tracks_stats()` 增加方向过滤（默认只看多头），
  并返回 `by_direction` 明细。
- **「真·零成交量」这个最该判沉寂的极端，永远进不了沉寂**：判据写成
  `(d.get("rvol_d",1.0) or 1.0) <= 0.6`，而 `_vr()` 在基准均量为 0 时返回 `0.0`，
  `0.0 or 1.0` 求值为 `1.0` —— **双重吞值**。改为显式判空（测得 0 参与判定、未测到 fail-open 不参与）。
- **控盘指纹系统性漏报**：确认层只覆盖前 24 个候选，其余行拿不到因子。前端在
  `oi_usd == 0 且 fut_qv > 0` 时显示「控盘未测」，把「没测到」和「真没有」分开。
- **`liq_5m` 恒为 0.0 与「真没爆仓」完全无法区分**（它被用在 reasons / 评分 / 反转判据三处）：
  新增 `market_ws.liq_available()`，**判据是「收到过帧」而不是「连接建立」**，不可用时
  `quote=None` 并贯穿到行级与面板级提示。⚠️ 这一条是**故意不接通**：档三实测强平流长窗
  **109 秒 0 帧**、REST 兜底 `allForceOrders` **404** / `forceOrders` **401** → **无公开兜底**，
  所以正确动作是**把它标出来**，而不是编一个数。
- **并发扫描没有锁**（各自跑满约 450 次 REST 扇出）：新增扫描级锁 + 等锁后双重检查，
  拿不到锁且无缓存则报错，不无限阻塞。
- **过滤层 `if b:` 整体包裹**：拿不到日线时，新币 / 刷量 / 假量**三道防线全跳过**。
  拆成两条显式路径，无日线时改用 `exchangeInfo.onboardDate` 判币龄，并新增
  `excluded_no_bars` 计数让「被跳过」这件事可见。
- **登记快照 5 列 → 14 列**（taker / oi15 / top / top48 / glob / liq5m / liqside / basis / spread），
  并新增 **C6 本地时序库**（`ts_series`）与 **C5 回放框架**（`src/radar_replay.py`）：
  回放**强制样本外切分**（前 70% 调参 / 后 30% 验证）、`sweep()` 按样本外 PnL 排序、
  「未测到」的轴 fail-open 但**逐轴计数**写进 `unknown`、结局价缺失的样本单独报
  `n_pnl_unknown` 而不进 PnL 汇总。全程**只算不改**（不写库、不发请求、不动规则）。
- **「命中 0 个」从此看得见**：新增阈值命中率监控（9 条规则 + 逐行累计），
  判死规则的门槛是「样本 ≥ 200 行且 0 命中」（样本不足不诬告），前端在 `dead_rules` 非空时告警。
  这套监控与生产**共用同一批常量对象**，防止两边各写一份字面量而漂移。

- **成本模型首次落地（#10）**：`sim_cost()` / `sim_pnl_net()` —— 双边 taker 0.05% + 双边滑点 0.15%
  （当时是**保守常数**，非实测）+ 资金费率按周期累计，名义 100U×10x → 空手往返 **4.0 USDT**。
  ⚠️ **`_sim_pnl` 一字未改、仍返回毛值**，净值**并列新增**（事件多 `pnl_net_usdt` / `cost_usdt`，
  复盘 Markdown 多一行「含成本估算」，前端在括号里补净额）—— 若拿净值**悄悄替换**毛值，
  历史样本与新增样本就会变成两个口径，而界面上看不出区别。**本版 ②③ 修的就是这个模型里的
  两处系统性偏乐观**（结算周期写死 8h / 滑点标定闭环断裂）。

**档三实测推翻了原有判断**（这是这一波最值钱的部分）：`ticker/24hr` 766 个 symbol
**含 `count` 但不含盘口**；`premiumIndex` 900 个 symbol 全字段可用（BTCUSDT 基差 −4.56 bps）；
`fstream` 域**只有 `@depth@100ms` 推流**（0.6~1.1s 首帧），`@aggTrade` / `@markPrice@1s` /
`!forceOrder@arr` / `!miniTicker@arr` **全部不推**（18~25s 超时，重试两次一致），
而 spot 域 `@aggTrade` / `@depth20@100ms` 正常。

**验证**：新增 `tests/test_v169_radar_remediation.py`（25 组 / 147 条断言）与
`tests/test_v170_ts_replay.py`（108 条断言），并**重写 `test_v164_oi_snapshot.py` 两处脆弱断言**
（原实现直接匹配 SQL 里的 `") VALUES"` 与一整段列名子串，属**排版耦合** —— 扩列折行后
一个 FAIL、另一个直接崩）→ 改为把相邻字符串字面量拼回一条 SQL 再解析，
并**新增真实落库往返护栏**（静态数个数查不出列顺序错位，只有真落库读回才能发现）。
**11 组 + 12 组变异验证全部转红**。

### ⑦ 仍然没做的，以及为什么
- **链上筹码分布 / 清算热力图 / 社交叙事**：需要外部数据源或凭据，本项目「零 Key、全免费端点」
  的约束下无法落地。其中「清算热力图」在免费路径上**已被实测否掉**（币安强平流不推、
  REST 兜底 404/401）—— 是**试过并确认不可行**，不是没试。
- **重新评估「只做多」/ 重新评估「预测启动」前提**：这是**产品定位**问题，需要你拍板。
  「可复现回放」这个前置件已经做出来了，但时序库从这一版才开始攒，**现在仍没有足够样本**。
...
- 雷达 v2 的「确认层」（OI / 大户比 / taker / 爆仓）**只覆盖前 24 个候选**
  ...
  实测 **55 只里 32 只没有任何确认层因子**。
- 更糟的是雷达用 `cf.get(k, 0.0)` 兜底，把这些行写成了**字面 0.0** ——
  消费方分不清「真的没变化」和「压根没测」。燃料轴在这些行上只能看到资金费率，
  于是**永远输出「中性」**：这就是「每只币都一样」的机制。
- **修**：妖币引擎分析的是**单只币**，按需补一次确认层（3~4 个 REST 调用，180 秒缓存，
  单次请求代价可忽略）。顺带把被兜底成 0.0 的 OI 一律按「没测」处理
  （顶层 `oi_chg24` 本来就是诚实的 `None`）。
- 补到 `oi_last` 之后还要**重算控盘指纹** —— 否则「换手畸高」（合约成交 ÷ 持仓 > 10x）
  的分母是 0，**永远不成立**，那一半行只能停在「未见控盘指纹」。

### ② 文案相同：同一个档位所有币共用一句死文案
- 37 只「中性」的币，`reading` 是**同一个字符串**；而卡片只显示档位 + 这句话，
  **看不到到底命中了什么** —— 视觉上当然「每只币都一样」。
- **修**：两轴的文案里带上**实测读数与命中项**。现在同为「中性」的两只币，一只写
  「费率 +0.005% · taker 0.40 · RVOL15 0.2x · OI15 −0.9% · 大户比 2.71」，
  另一只写「费率 −0.012% · taker 1.12 · RVOL15 1.9x · OI15 +0.5% · 大户比 1.47」。
- 「未见控盘指纹」也不再说空话，而是**列出查过哪五项**：
  「未命中 无现货 / 合约独大 / 换手畸高 / 空头付钱 / 拉升无爆仓」；
  命中时给出该指纹的**人话解释**（如「合约独大（现货没深度，少量资金就能推价）」）。
- 记 0 分的条目（如大户拥挤）单列成「**关注**」—— 它不构成燃料、不改档位，
  但「检测到了却不显示」正是「看起来什么都没发生」的来源之一。

### ③ 死规则：`taker >= 1.85` 从来没触发过
- 实测 40 个最高成交额合约的 `takerlongshortRatio`：min 0.439 / 中位 1.110 /
  p90 1.426 / **max 1.790**，`≥ 1.85` 命中 **0 个**。而这条信号同时挂在**四个地方**
  （`_stage_of` 的点火判据、`_radar_score` 的 +3、雷达依据文案、燃料的「买盘主导」）——
  也就是说它**自上线起一次都没生效过**。这不是「市场没出现」，是阈值压根够不着。
- **修**：改成 **1.30**（主动买 ≈ 主动卖的 1.3 倍，实测命中 10/40 = top 25%），
  四处一起改，并落成命名常量 `_TAKER_BUY_DOMINANT`（monster 侧镜像 + 测试钉住不许漂移）。
- **⚠️ 这是一处会产生行为变化的修正**（点火判定更易命中、评分 +3 可能开始加分），
...
- 新增档位 **「未能判定」**：确认层没补到时不报「中性」，而是明说
  「这是『没测』，不是『没有燃料』」。
- **只有现货、没有合约市场**的币（如 SPCXBUSDT）单独措辞：OI / 持仓额 / 大户比 / 费率
  **本就不存在**，不是取数失败 —— 但量能这类现货口径也有的读数照常给出，
  该给「有油」就给「有油」。

### 验证
- 新增 `tests/test_v167_monster_factors.py`（4 组 60+ 断言）：阈值镜像与「源码里 1.85 必须消失」、
  补取的命中 / 缓存 / 降级三态、`oi_usd` 重算、「没测 ≠ 中性」、**同档位两只币文案必须不同**、
  现货口径与合约口径措辞必须分离。
- 既有 `test_v166_monster_engine.py` 全过；**全量 23 个套件**通过；`tsc --noEmit` 退出码 0。
- 真机实测：55 只里 32 只走补取；`买盘主导` 命中从 **0/49 → 4/49**；
...
### 那个叠字长什么样
- 剧本进度带上的三角游标原本画在**高亮框外侧上方**，用来指「当前格」。
- 问题出在「当前格」恰是**第 1 格或第 2 格**时：游标正好落在左上角那行表头
  「妖币剧本 · 演到第几格」上面，把中间的「·」和后面的「演」糊成一团。
- 偏偏**吸筹（第 1 格）就是引擎最常给出「可埋伏」的那一格** —— 露脸最多的图正好带着这个瑕疵。

### 改了什么
- 游标改为贴在**高亮框内侧顶部**、指向框内，与表头在纵向上彻底脱开。
- 只动妖币引擎的进度带绘制函数（`src/square_monster.py` 的 `_draw_stage_band`）。
  封面的「90d 高/低」「现价」标注、三轴卡、出场四段、正文文案**都没动**；
  代币那套 SMC 引擎**一行没碰**。
...
**核心**：你说「分析代币和分析妖币生成的广场内容要用不同的技巧」——这一版把它做成了**两套引擎**，
不是同一个工具的两组参数。代币继续用 SMC；妖币另起一套**剧本三轴**（位阶 / 控盘度 / 燃料）。
顺带把清单里挂着的两个决策**仪表化**了：不再是「等样本」，而是随时可问「还差几笔」。

### 为什么必须分开：SMC 的前提在妖币上不成立
- SMC（结构、BOS・CHoCH、OTE、OB、FVG）成立的前提是**「价格由众多参与者的共同行为堆出来」**——
  结构位、订单块之所以有用，是因为那里真的有人挂单、真的有共识。**代币满足这个前提。**
- 妖币不满足：**控盘盘的 K 线是画出来的**。你看到的 OB 就是诱多区，你看到的「扫流动性」
  就是庄家专门去点你止损的那一下。**用 SMC 分析妖币，等于拿散户的地图去找庄家的门。**
- 所以妖币看的是**控盘意图**，三轴：
  **① 位阶**（庄家剧本演到第几格）· **② 控盘度**（盘在谁手里）· **③ 燃料**（往上推的油从哪来）。

### 剧本三轴：位阶一票否决，控盘度不等于坏消息
- **位阶**是七格主剧本：吸筹 → 点火 → 已拉升 → 垂直拉升 → 派发顶部 → 崩跌 → 沉寂。
  只有**吸筹 / 点火且不晚**才允许进场；做空埋伏与「异动」是旁支，直接不进。
- **「晚不晚」直接复用雷达自己的两条闸门**（价格硬挡线 10% / OI 堆积线 5%），**不另发明口径**——
  否则稿子里说的位阶会和你在 app 里看到的对不上。
- **控盘度不是坏消息**：命中「无现货 / 合约独大 / 换手畸高」说明「剧本是真有人在演」，
  这恰恰是妖币会涨的原因。真正的含义是**离场必须果断**——控盘盘没有自然买盘接你的货，
  你以为的支撑是画出来的。
- **燃料全是「对手盘的痛」**：空头付钱（负费率，最猛的燃料）、OI 蓄力、大户拥挤、爆仓结构。
  其中「费率自极值回落」与 `radar_tracker._reversal_now` **完全同口径**。

### 出场不改一个数：四段全部镜像 radar_tracker，并写明一个难听的事实
- 认输线 **逆向 10%** / 达标线 **顺向 25%** / 移动止盈 **自极值回撤 12%** / 提前落袋 **反转因子命中**——
  四个数与 `radar_tracker` 的 `FAIL_HIT / GAIN_HIT / TRAIL_PCT / _reversal_now` **逐一对应，有测试钉住不许漂移**。
- **且把一句实话写进稿子**：在 10x 下，10% 止损**就是强平线** ——
  100U 本金 ×10x = 1000U 名义，逆向 10% 正好是 100U，**打到止损等于本金归零，不存在「小亏一点」**。
  这条线不是「保护你」，是**逼你认输**；能调的只有仓位，不是这条线。这不是软件不够聪明，是杠杆的算术。
- **妖币用固定 10% 止损而不是结构位**，原因和上面同源：控盘盘的「结构失效位」是画出来的，
  挂结构止损等于把止损位送给庄家。

### 只做多，绝不做空；不给点位也是结论
- 安装版真实战绩：3 笔做空**全部被轧空击穿 10% 强平线，最大有利仅 0.0% / 0.0% / 1.4%**——
  **从一开始就没跌过**。妖币的顶常是二次拉升前的换手，做空你以为在赌回调，其实是给庄家当接盘的人。
  所以 `plan.direction` **写死 long**，源码里不存在给空头出计划的路径。
- **位阶不在允许窗口时，稿子明说「本次不给点位」**，`plan` 写 `null`——绝不硬造方向。
  副作用是正确行为：`paper_orders` 那时不会建单，**这不是漏单**。

### 接线：两个端点 + 一个新技能 + agent 路由分流
- 后端：`/api/square/monster/compose`（出稿）与 `/api/square/monster/analyze`（只看结论不出图）。
  注释里写明与 `/api/square/rich/compose` 是**两套引擎，不是同一套的参数**。
- 新技能 `square-monster-post`：4 套**妖币专属**风格（剧本拆解体 / 埋伏笔记体 / 别接盘体 / 说人话体），
  与代币引擎的 4 套（冷静复盘体 / 交易员日记体 / 问答体 / 直给体）**刻意不重名**——
  名字不同是为了让 agent 不能顺手互换。
- 产物 **`cover.png`（90 日走势 + 剧本进度带 + 三轴卡）** 与 **`chart_playbook.png`（1080×1080 竖版剧本卡，
  手机可读：结论 + 进度带 + 这一格/下一格 + 控盘与燃料 + 出场四段）**；
  产物结构与代币引擎**同构**，`square-post` 发布链路**没改一行**，`paper_tracker` 也能直接读 `plan`。
- Agent 提示词改成**先按标的分流**：普通代币 → `square-rich-post`（SMC）；妖币 / meme / 百倍币 / 雷达里的异动币
  → `square-monster-post`（剧本）。并写明**两者绝不可互相替代**；
...
- **决策 2（设不设最低分门槛）** 与 **决策 3（挡不挡「登记时已下跌」）** 此前写的是「等 OPT-04 后的新样本」——
  但**没有人会记得回来复查**，挂着等是最糟糕的形态。现在做成 `state.radar_recheck_decisions()` + agent 工具
  `meme_watch(mode='audit')`：每次调用重算证据，样本不够**如实说还差几笔**，够了**直接给结论**。
- **决策 2 的关键改进**：先**剔除 OPT-04 会挡掉的那批样本**再看 score 还有没有判别力——
  不剔除的话 score 只是 chg24 的影子（高分样本恰好都是「已涨」的 dump），属于**重复计数**。
  函数会同时给出「全部样本」与「OPT-04 之后」两张分桶表，让这件事看得见。
- **它只给证据，不改任何规则**——加闸门是产品决策，要你拍板。测试里钉了这条。

### 验证
- 新增 `tests/test_v166_monster_engine.py`（10 组、90+ 断言）：引擎分离（**monster 不得 import 任何 SMC 分析函数**）、
  常量镜像逐一相等、位阶一票否决、合议优先级（重度控盘 + 满油但位阶在派发 → 仍离场）、
  只做多、不给点位、四风格不重名、产物同构、符号不在视野内报错、决策复核 waiting↔ready 双向、
  接线（提示词 / 工具 schema / 技能发现 / 前端文案）。**全量 22 个套件通过，`tsc --noEmit` 干净。**
- **真实数据端到端跑过**（不是只有单测）：拉真雷达 → `EIGENUSDT` 判定「吸筹·第 1/7 格 → 可埋伏」，
  出稿 + 两张图（75KB / 98KB）落盘，`plan` = 入场 0.1908 / 止损 0.1717（= 入场 ×0.9）。
- 视觉验收发现并修掉三处：面积渐变把曲线下填色整片盖掉（改成先铺底再填）、
  三轴卡文字超行**截半句话**（改成带省略号）、价格线压在高/低标签上读不出来（改成带底衬标签）。

## 🆕 v1.5.63 更新要点（妖币雷达第二批落地：登记门槛分段 + 假启动提示 + 撤单与保护单）

**核心**：把登记清单 `outputs/todo-v1.6.4.html` 里能落地的项**全部落地**（10 条编号任务 → 9 条已落地，
第 10 条是「装上去」这个动作，得你来）。**止损 10% 与 10x 杠杆一律不动**——这一批改的全部是「怎么找妖币」和「怎么把单管住」。

### 登记门槛分段：硬挡线 3% → 10%，3~10% 只降分不挡
- 先把一条**我们自己写错的话**改掉：原注释说「连 3%~12% 这段也晚了」，**数据不支持**——
  9 笔「登记时已涨」样本的 `chg24` 最小是 **11.3%**，其余 13.9%~21.9%，**3%~11.3% 之间一笔样本都没有**；
- 所以有依据的只是「**≥11.3% 该挡**」，于是硬挡线定在 **10.0**（`_TRIG_LATE_CHG24`），
  `3%~10%` 段**降 6 分、不挡**，交给 OI 判据二次筛；触发线 `_TRIG_UP_CHG24 = 3.0` **不动**，两条线从此解耦；
- 诚实说明：**这次放宽是样本中性的**（该段 0 笔样本），测试里留了「3%~10% 段样本数 = 0」的断言把这件事钉住；
- 还要说清一个事实：**登记链路不看 score**，所以「降分」的作用是排序 + 把这件事写进依据便于日后分桶回放，
  **真正的闸门只有价格硬挡线 + OI 堆积**。

### 持仓质量提示「疑似假启动」（只提示，绝不自动平仓）
- 跟踪 ≥24h 且**顺向历史极值** < 5% 的行打标，Tracks 面板出红色徽章；
- 真实口径（写清分母，别把数字读成另一个意思）：**活过 24h 的 10 笔 dump 里 7 笔被标，3 笔 moon 0 笔被标**
  （17 笔 dump 里有 7 笔在 24h 前就逆向破 10% 关单，走不到被标记那一步）；
- **为什么绝不自动离场**：VTHO 就是反例——早期回撤 7.9% 之后冲到 **+31.3%**。徽章只是提醒，止损与杠杆一律不动；
- 顺手修掉一个真 bug：第一版对**做空**也读 `max_gain_pct`，而做空的顺向是「跌」——
...
- 胜率分母**只用已关单**（进行中既不算赢也不算输）；
- **不删、不降权**：4 笔样本可能纯属偶然，先分开摆在战绩面板上，攒到 10 笔再决定。

### 撤单 + 保护单：把交易闭环补上
- **撤单**（此前整条链路只有「下」没有「撤」）：签名 `DELETE /api/v3/order`，
  订单台账同步置 `CANCELED`，订单跟踪卡出现撤单按钮（只对「还在交易所挂着的委托单」显示）；
  交易所返回 `-2011`（订单已是终态）**按成功收敛**——「别让它再挂着」这个意图本来就已经达成，报错只会诱导重试。
  原来的 `DELETE /api/orders/track` 只是**本地不再跟**，两个动作的说明已经分开写清；
- **保护单**：下单即挂 OCO，止损 **−10%**（与你定的模拟口径完全一致，刻意不做成可调参数）。
  只有 `signal.protect=True` 且人工确认过才发第二张单，确认框里会显示「会不会挂保护单」；
  **挂不上不算下单失败**，只附一条 `protect_warning` 让你手动补；
- 顺便抓到一个静默 bug：`executor.place_oco_order` 一直在传**非法的 `legs` 数组**
  （Binance OCO 要的是扁平参数 `price` / `stopPrice` / `stopLimitPrice`），而且它**从来没有任何调用方**，
  所以这个 bug 挂了几个版本没人发现。已改成正确参数并委托给 `cex_wallet`（统一密钥 + 交易规则取整 + 错误归因）；
- 台账口径写清楚：`radar_tracks` = 雷达模拟仓、`paper_orders` = 广场发文纸单、`tracked_orders` = 真实委托，**三者不混账**。

### v1.6.4 的四项（同批）：纯合约币取价兜底 + 登记快照落库 + OI 判据 + 费率降权
- 候选池扩到合约全市场后，**只上合约、不上现货**的币（RAVE / LAB 这类）用现货永远取不到价 →
  永久卡 pending、7 天后被静默 expired。改为三级兜底：现货 WS → **合约 WS** → 合并快照（**现货优先**，
  不让合约价差污染现货币）；
- `radar_tracks` 补 5 个**登记时刻数值快照**列（`snap_chg24/oi24/amp24/funding/rvol15`）：
  此前回放只能解析 `reasons_json` 文本反推，同一条规则两次算出不同答案。**缺值写 NULL 不写 0**（口径不能混）；
- 新增 **OI 判据**：`OI 24h` 为正的样本 **9 笔 → 9 笔全 dump**，3 笔 moon **从无正值 OI**（**零误杀**）。
  登记条件由「价格未启动」扩为「价格未启动 **且** 持仓未堆积」，阈值取 5.0 是**故意的零外推**
  （依据生成处本身就是 `abs(oi_chg24) >= 5`）；
- 「空头付钱」扣分 **10 → 3**：VTHOUSDT 费率 −0.776% 却是 +31.3% moon，判别力弱；风险提示语义保留。
...
**核心**：用户定位「止损 10% 与 10x 杠杆**都不动**，亏的根因是**找妖币的方法错了**」。
按安装版的真实关单数据回放，把雷达从「波动率探测器」改回「妖币探测器」——
**只有尚未启动的币可以登记**，并且候选池从现货扩到**现货 ∪ 合约全市场**。

### 选币规则：登记即入场，且只登记「尚未启动」的币
- 数据依据（安装版真实关单 23 笔 = 20 多 + 3 空，胜率 13.0% / −1386.5U）：
  **3 笔 moon 的登记依据里全都没有「24h +X%」**（即登记时涨幅 < 3%），
  而 9 笔「登记时已涨 7.4%~24.8%」的**全都是 dump**；
- 落地：当日涨幅 ≥ 3% 的行**不再进登记组**，改划「已拉升」展示组（不删除，仍然可见）；
- 回放：20 笔做多 −1086.5U / 胜率 15.0% → **只留未启动 7 笔 +213.5U / 胜率 42.9%**；
- 不设「候选观察期」（用户投票否决），过滤条件全部取自**登记时刻的快照特征**。

### 绝不做空 + 负费率不再硬阻断
- 3 笔做空 **3/3 全亏**（CAKE +10.0% / THETA +10.3% / SAGA +11.0% 被轧空），
  而**最大有利仅 0.0% / 0.0% / 1.4%** —— 从一开始就没跌过；
- 做空埋伏（SHORT_AMBUSH）**移出登记组**，只在「顶部风险」展示组保留可见；
- 负费率**不改方向、只标注并扣分** —— 反例 VTHO 费率 −0.776% 却是 +31.3% 的大赢家：
  合约里空头拥挤会把费率压负，而低负费率恰恰是**挤空燃料**；费率评分同时由 `abs()` 改为方向化。

### 候选池扩到合约全市场（纯合约妖币终于可见）
- 此前池子只取自**现货**快照，而 RAVE / LAB 都是**只上合约、不上现货** —— 排序规则再准也看不见；
- 现在池子 = **现货 ∪ 合约**，按 `max(现货成交额, 合约成交额)` 排序取前 110；
- **代币化股票/大宗必须挡在池外**：合约全市场里混着近 200 个 `TRADIFI_PERPETUAL`
  （NVDA / TSLA / XAU / XAG / BZ / QQQ / SPY / SKHYNIX…），实测会把 110 个池位中的 57 个
  占成纯合约、且一大半是 TradFi。判据取合约接口的 `underlyingType == "COIN"`
  （权威字段，随上线自动更新）；**Alpha 币（LAB 就是）与 meme / 中文盘全部保留**；
- 纯合约币的 K 线走合约回退，否则会在「日线不足 25 根」与「15m 长度 < 30」两处被静默丢弃；
- 生产接口实测：池子 86 现货 + 24 加密纯合约，**TradFi 零泄漏，LABUSDT 已进池**。

### 控盘代理层（链下、零新增外部依赖）
- 公开接口拿不到持币集中度（`query-token-audit` 只有蜜罐/税率，`token-info` 的 `holders`
  是**持币人数**、不是集中度）→ 改用四个可算指纹：
  `无现货` / `合约独大`（现货成交占比 < 5%）、`换手畸高`（合约成交 ÷ 持仓 > 10 倍）、
  `空头付钱`（费率 ≤ −0.15%）、`拉升无爆仓`（涨 ≥8% 但 5m 爆仓 < 5 万，并带数据可用性守卫）；
- 命中即扣分，雷达行出**红色「控盘」标记**、悬停显示具体原因；
- 生产实测命中：AKE 无现货 + 换手 12 倍、LSK 换手 29 倍、IOST / CVC / STEEM 空头付钱。

### 验证
- 新增 `tests/test_v162_radar_select.py` 与 `tests/test_v163_manip_guard.py`
  （触发方向化 / 评分去绝对值 / 只登记未启动 / 绝不做空 / 控盘四信号 / 扩池与 TradFi 过滤 /
  AST 与链路护栏）；`test_v162` 的分组断言同步更新；
- **全量 19 套件全绿**，`tsc --noEmit` 退出码 0，生产接口冒烟通过。

## 🆕 v1.5.61 更新要点（广场发文模拟挂单：文章观点自动建档、7 天定胜负）

**核心**：富媒体发文成功后，按文章 SMC 计划**自动建模拟挂单**（100U 保证金 × 10x 杠杆，
不碰真实资金），后台按 5m K 线驱动状态机，7 天观察期到期强制结算——
广场页新增「模拟挂单」标签页，一眼看清每篇文章的观点到底赚没赚。

- **建单链路**：compose 把 plan 节（方向/入场区/止损/止盈/进价）写进 meta.json →
  发布成功入账钩子 `_paper_order_from_run` 建单（agent / 手动两条路径汇聚，run_dir
  唯一索引幂等）→ `paper_tracker` daemon 线程 60s 复查；
- **状态机**（纯函数 `_eval`，可离线单测）：pending（挂单中）→ open（已入场）→
  win / loss；同根 K 同时触 TP+SL 保守判 loss；7 天未入场 → noentry；
  盈亏按 10x 杠杆折算保证金收益率（%）；
- **观望不建单**：neutral / noplay（止损距离超 8% 护栏）的文章 plan=null，绝不硬造方向；
- **广场页双标签**：「发文台账 | 模拟挂单」切换，7 张统计卡（总单/挂单中/持仓/
  获利/亏损/未入场/胜率）+ 订单卡列表（现价、入场区、止损止盈、盈亏着色），
  支持单条删除（二次确认，仅本地数据）；
- 新增 `GET /api/square/paper` 与 `POST /api/square/paper/delete` 路由，
  启动钩子 + 路由双保险拉起后台复查线程。

回归测试 test_paper_orders 17/17 全绿（_eval 状态机直测 + 源码护栏）。

## 🆕 v1.5.60 更新要点（入场结构位止损距离护栏：10x 杠杆活着到止损的前提）

**用户实测两篇翻车**（LSK：旧 OB 距现价 -85% 还挂单；牛来：空头 OB 区宽 58%），
拍板定版：

- **≤8% 护栏**（`_stop_ok`）：入场 → 止损距离必须 ≤8%（10x 强平线 ~9% 的前提），
  超限的结构位一律不用；结构位降级链 **OB → OTE → 近10根摆动点 → 观望（noplay）**，
  空头镜像；noplay 时只输出观望劝退行，不带仓位算法；
- **「我的看法」与上文自洽**：4h 结构与最终 bias 方向相反时必须说破级别分歧
  （如「日线大趋势分量更重 权重3:1」），禁止上下文打架；
- **老结构如实标注**：OB 距现价超 25% 时标注「很远的老底/老顶，短期参考意义有限」；
- `_pick_tp` 裸 None 解包 bug 修复（stat 无 k90 时 cands 混入 None）；
- 测试同步：sizing 7 个过时用例更新到护栏口径（锚位断言 5→6 处）、
  新增护栏测试（暴涨/暴跌合成行情 → noplay）、
  修复 test_v1541 的 `sys.modules["state"]` 桩在 pytest 收集期污染全局导致的
  test_v1535 e2e 假失败（v1.5.41 起的存量问题）。
...
- **日线大趋势「趋势延续优先」**（v1.5.57，新增 `_htf_trend`）：锚定最近显著摆动点的
  **实体边缘**（下沿/上沿），**实体 K 收盘破位才翻转**——影线捅破不算，
  中途高点走低只算上涨中的调整；4h 图左上角标注「大趋势·日线 多头/空头/方向不明」；
- **反向 OB 两态显示**（v1.5.58）：
  - 已实体收盘破结构最低/最高点 → **实线框**标「CHoCH 空头OB / CHoCH 多头OB」；
  - 尚未破 → **虚线框**标「跌破后的空头ob位 / 升破后的多头ob位」，
    作为价格若跌破/升破后的反向入场参考点（用户指定：也要显示，虚线标出）；
- **OTE 口径改版**（v1.5.59）：最优入场窗口从 0.618-0.705 改为 **0.62-0.79**，
  一般用**中间值 0.702** 作入场参考位；新增 `ote_entry` 字段，
  计划入场价取 0.702 位；正文固定附「4h OTE 参考」行（窗口 + 中间值实际价），
  「我的看法」补 OTE 概念介绍；
- 4h 结构图 OTE 金框自动跟随新窗口范围。
...
- **布局**：价格区 216px → 380px（`y1 = H-236`），量区压至 66px（`vy1 = H-150`）；
- 网格、蜡烛实体、高/低胶囊、现价胶囊、信息卡样式延续 v1.5.54 定版。
