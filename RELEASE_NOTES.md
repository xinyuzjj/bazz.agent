...
## 🆕 v1.5.54 更新要点（封面 K 线终版：恢复虚线网格 + 大量柱区样式，用户以真实 BTC 样图拍板）

**用户拍板**：看过真实 BTC 数据渲染后，指定按 v1.5.51 风格样图定版（撤销 v1.5.53
「极简无网格」与 v1.5.52「压矮量区」）：

- 恢复横向虚线网格 + 日期锚点竖向淡网格、价格轴 SUB 色；
- 价格区 104 ~ H-320，蜡烛实体宽度 0.62 步长、矩形实体、影线收暗 30%；
- 成交量区恢复高排（H-100），量能起伏与价格走势对照更直观；
- 高/低胶囊标签、现价虚线胶囊、底部信息卡保留。

真实 BTC 行情实测（fapi 90d）：现价 77,176 / 90日 57,758~82,282 / 费率 +0.0081% /
OI 79.94 亿 / FNG 61，样图目视确认。回归 fallback 11/11、styles 17/17 全绿。

## 🆕 v1.5.53 更新要点（封面 K 线定版「极简无网格」风格，用户三选一拍板）

**用户反馈**：前两版样式仍不好看。出三种风格样图（霓虹渐变 / 空心经典 / 极简无网格）
供用户挑选，拍板 **C · 极简无网格**：

- 网格去掉虚线与竖线，只留极淡横线（24,28,34）+ 更柔的价格轴文字（110,118,130）；
- 蜡烛影线收暗至 45%（柔和衬托实体），圆角细身实体保持 v1.5.52 比例；
- 高/低胶囊标签、现价虚线胶囊、量区矮排、底部信息卡不变。

回归：fallback 11/11、styles 17/17 全绿；样图实测目视确认。

## 🆕 v1.5.52 更新要点（封面 K 线图比例修正：价格区拉高、量区压矮、细身圆角蜡烛）

**用户反馈**：蜡烛图太扁、不喜欢扁宽样式；量柱区域希望弄小一点。

- 价格区高度 296px → 380px（`y1 = H-236`），蜡烛纵向舒展；
- 蜡烛实体宽度 0.62 → 0.55 步长，圆角矩形绘制，去扁感；
- 成交量区高度 192px → 66px（`vy1 = H-150`），衬托价格主区；
- 最小实体高度 1.2px → 1.6px，十字星也能看清。

回归：fallback 11/11、styles 17/17 全绿；样图实测目视确认。

## 🆕 v1.5.51 更新要点（广场发文 K 线图美化：零新依赖，纯 Pillow 重绘）

**用户反馈**：生成的 K 线图希望更好看。封面主图与 24h 分时图全面重绘，仍是零新增
依赖（不引 matplotlib，纯 Pillow）。

### 封面主图（90d 蜡烛图）

- 背景纵向微渐变（深蓝黑 → 币安黑），告别死黑底；顶部金色描边呼应品牌色；
- 网格虚线化，日期锚点补竖向淡网格；区间高/低虚线 + 胶囊标签（描边着色，深底可读）；
- 蜡烛影线收暗一层，实体更立体；成交量柱收暗一档，衬托价格区；「成交量」标签移到左上；
- 现价横线虚线化 + 右轴价格胶囊；底部信息条升级为圆角卡片（label/value 分层）。

### 24h 分时图

- 面积填充改纵向渐变渐隐（贴线最亮 → 底部没入背景），mask 贴多边形；
- 折线三层描边（外圈辉光 → 中层过渡 → 亮芯），高低点光晕圆点 + 胶囊标签；
- 网格虚线化、现价线虚线化，与封面风格统一。

### 回归

- fallback 11/11（封面 `k90_label` 参数化护栏不破）、styles 17/17、sizing 16/16、
  invalidation 10/10 全绿；样图实测（SNDKUSDT 模拟数据）目视确认。

## 🆕 v1.5.50 更新要点（止盈瞄准 SMC 流动性区域：摆动点 → FVG → OB 块阶梯）

**用户指示**：止盈按 SMC 来不要乱改；但 SMC 目标太近、盈亏比不好时，要换其他止盈方法；
目标可以瞄向流动性区域——FVG、OB 块等地方。（现场触发：SNDK 空单止盈 1,620.06
距入场仅 0.4%，盈亏比 ≈0.3，用户质问「这个盈亏比是认真的吗」）

### 止盈阶梯（`_pick_tp`）

1. **首选 SMC 摆动点**（前高/前低 = 裸露流动性池），RR ≥1.5 直接用；
2. **太近（RR<1.5）→ 换其他流动性目标**，按近 → 远依次尝试，取第一个 RR 达标的：
   - FVG 缺口中位（价格对缺口有回补引力，标签「FVG 缺口回补」）；
   - 对侧 OB 块边缘（在止盈方向上才纳入，标签「上方/下方 OB 块」）；
   - 近 30 根 4h 极值 → 90日极值；
3. 标签如实标注实际采用的目标类型，不伪装成摆动点；
4. 全不达标取最远目标、RR 如实回报，由 `_plan` 决定「小仓/劝退」话术（RR<1 劝退、
   1≤RR<1.5 标小仓，v1.5.49 逻辑保留）。

### 回归

- `test_v1540_square_sizing.py` 扩到 **16/16**：摆动点达标不换 / 摆动点太近换 90日极值
  且标签如实 / RR<1 劝退 / 1~1.5 小仓注脚 / 强 RR 无注脚；fallback 11/11、
  styles 17/17、invalidation 10/10 全绿。

## 🆕 v1.5.49 更新要点（止盈呈现诚实化）

- RR 从**入场参考价**如实计算（此前按现价算，挂单价接的单盈亏比全失真）；
- RR<1（目标贴入场、止损远=结构质量差）→ 如实劝退且不再给仓位算法；
- 1≤RR<1.5 → 标「盈亏比一般，只试小仓」；RR≥1.5 → 正常计划。
...
### 止损：回归纯 SMC 结构（用户多轮实测校准）

- **止损只锚结构失效位，不凑风险数**：多头 OB 入场 → OB 下沿下方 0.5% 缓冲；
  空头 OB → 上沿上方 0.5%；OTE → OTE 沿外侧缓冲；现价入场 → 摆动极值外侧。
  每条止损都带**锚位说明**（「结构失效即离场」），废弃 v1.5.46 的「风险预算反推」
  （那等价于恒亏 10U，用户明确否决）。
- **进价规则定版**（用户指定）：**多头买在 OB 上沿**（回踩第一触点，保证成交）、
  **空头卖在 OB 下沿**，文案标明具体进价；止损锚在对侧沿 + 缓冲。
  距离 = 区宽 + 缓冲，从进价算，不用现价。
- **仓位回归演示口径**：100U 本金 = 100U 保证金，10x → 名义 ≈1000U；
  打止损亏损如实报数（官方口径：数量按交易所步进取整），超本金 10% 时附
  「降名义」参考建议——**止损位永不为凑数而动**。
- 官方对账：78,542 做空、止损 78,934.71 → 用户官方计算亏 ≈4.7U（步进取整），
  理论 ≈5.0U，文章口径一致。

### 标题：按爆款公式重写标题池（v1.5.47e）

- 调研爆款标题写法落地：**痛点拷问 / 数字+亲历教训 / 悬念留白 / 对比反差 /
  身份共鸣 / 犀利引语 / 反常识** 七类公式；四风格共享 `_pick_title`，
  方向池各 12~14 条随机抽，同币连发不重样；
- 结构事件标题（下影线扫止损 / 冲破前高套多少人 / 0.618 位置到了 /
  空头 OB 压顶）命中时 60% 概率优先——细节感强；
- 全部 ≤25 字、币种关键词前置、不承诺收益（合规）。

### 回归

- `test_v1540_square_sizing.py` 11/11（SMC 止损 + 官方对账口径 +
  「恒亏/反推/放弃」旧措辞守卫）；`test_v1540_square_styles.py` 17/17
  （标题池多样性 / 事件标题必进候选 / 防串币）；
  `test_v1539_square_invalidation.py` 10/10、`test_v1545_kline_fallback.py` 8/8。

## 🆕 v1.5.46 更新要点（仓位算法按 SMC 止盈止损重设计：结构定止损 · 风险预算定仓位）

**用户实测反馈**（v1.5.45 文章）：计划写「挂 0.1155~0.128 多头 OB 区回踩接（限价）」，
仓位行却算出「止损 0.1149（距离 13.7%）、打止损亏 ≈ 137.2U（占本金 137%），想稳一点
把杠杆降到 1x」——止损不对，没有按 SMC 的止盈止损来设计。

### 根因（两层硬伤）

1. **止损距离用现价算**：计划明明是挂 OB 区限价接，却按现价 0.1332 算出 13.7% 距离；
   真按计划在 OB 区（区中位 ≈ 0.1218）进场，距离只有约 5%；
2. **名义写死 本金×10x = 1000U**，与止损距离完全无关 → 亏 137% 后只好建议
   「降到 1x」——把整单推翻，等于没设计。

### SMC 正确顺序：先结构、后仓位

- **止损锚在结构失效位**（原 v1.5.39 设计保留）：OB 入场 → OB 下沿下方 0.5% 缓冲；
  OTE 入场 → OTE 下沿下方；现价入场 → 近 10 根摆动低点下方。
- **入场参考价**：挂区间限价时取区间**中位**（OB 中位 / OTE 中位），现价入场取现价。
- **距离从入场价算**，不再用现价。
- **仓位由风险预算反推**：单笔风险 = 本金 10%（10U）→ 名义 = 10U ÷ 止损距离；
  杠杆 = 名义 ÷ 本金，**封顶 10x**。止损贴结构（≈0.5%）时自然到 10x/1000U 的演示口径；
  止损宽时名义自动缩小，**打止损恒亏 ≈10U**；
  距离宽到连 1x 都装不下（>10%）→ 如实建议**放弃这笔**，等更贴近结构位的入场。
- **止盈行补盈亏比**：第一目标相对入场价的盈亏比 ≈ N（风险回报比一眼可见）。

### 验证

- `tests/test_v1540_square_sizing.py` 按 SMC 风险口径**整体重写为 11 项判据**：
  贴结构止损封顶 10x/1000U / 正常止损风险反推 / 宽止损自动缩仓 / >10% 建议放弃 /
  距离必须从入场价复算（OB 素材：入场价距离 < 现价距离）/ 止盈带盈亏比 /
  旧文案（含用户贴的那行原文）必须被拒 / 源码护栏（名义=风险÷距离、禁写死 本金×10x）/
  端到端止损与反向剧本一致。**11/11**。
- 既有套件全绿：styles 14/14、invalidation 10/10、kline_fallback 8/8；`py_compile` 0。

---

# BAZZ.AGENT v1.5.45

## 🆕 v1.5.45 更新要点（广场 rich 稿 90 日 K 线不足 → 4h/1h 降级，保证出稿）

**用户实测反馈**：新上市/数据源异常的币，90 日 K 线数据不足，rich 稿出图直接失败。
要求：90d 不够就换 4h，再不够换 1h，**保证出稿**。

### 降级链：1d×90 → 4h×540（≈90日）→ 1h×720（≈30日）

`_collect` 取日线后检测可用性（≥5 根且非占位平线），不可用就依次降级：

1. **4h×540** —— 覆盖约 90 日，粒度更细；
2. **1h×720** —— 覆盖约 30 日，新上市币最后的兜底。

三档全空（币种刚上线几分钟或已下架）才返回带指引的错误，不再吐裸异常。

### 文案跟随实际周期，不再写死「90日」

降级后 `k90_label` 记录实际覆盖范围，封面图的「90d 高/低」「90日区间位置」
自动变成「近90日·4h 高/低」「近90日·4h区间位置」；文章走势段如实说
「这近30日·1h基本就是…」；页脚去掉写死的「90d 日线」。
**数字全部照实计算，只是换周期，绝不伪造数据。**

### 验证

- 新增 `tests/test_v1545_kline_fallback.py` **8/8**：充足不降级 / 1d 不足降 4h /
  4h 不足降 1h / 平线占位触发降级 / 三档全空不伪造 / `_k_usable` 阈值 /
  封面与文章文案跟随 label（源码护栏）/ 空数据拒绝出图且错误文案含降级链。
- 既有 square 套件全绿：styles 14/14、sizing 12/12、invalidation 10/10；`py_compile` 0。
...
**用户实测反馈**（v1.5.42）：① 对话框里模型选不了；② 「拉取模型」拉回的几百个模型
铺成一整堵 chip 墙，改成下拉式；③ 对话框模型下拉应该只显示「大模型 LLM 配置」里
选择了的模型。

### 一、对话框模型下拉：只显示设置里选了的

ChatView 原实现把「拉取模型」拉回的**厂商全量目录**（Nous 一家就是 300+）全塞进
会话模型下拉，几百项根本没法选。现在只合并三类**你显式配置过的**模型：
主模型 + 备用 fallback 链 + aux 槽位（深度思考/视觉/摘要），去重后作为下拉选项。

### 二、设置页主模型：datalist → 下拉选择

主模型原来是文本框 + datalist 联想，几百个模型时基本选不动。改成 `<select>` 下拉，
选项 = 拉取目录 + 厂商预设 + 当前值（当前值不在清单里时保留为独立选项，防丢配置）。

### 三、备用模型：按钮墙 → 「下拉添加 + 已选 chip」

原来把目录全量铺成 chip 墙（几百个按钮）。现在：一个「＋ 下拉添加备用模型」的
下拉框负责加，已添加的模型显示为 chip（点击 ✕ 移除，主模型带 ★ 标记）。
aux 三个槽位同步改成下拉（原来引用的 datalist 已随本次改造移除）。

### 验证

- 新增 `tests/test_v1543_model_pickers.py` **4/4**：ChatView 禁止再拉全量目录 /
  主模型必须是 select / 备用必须「下拉添加+已选 chip」且禁止按钮墙 / i18n 双语齐全
  且废弃键已清理。
- `tsc --noEmit` 退出码 0；`vite build` 成功。
...
这一版集中修广场自动文章（square-rich-post）：上一版你指出「反向剧本自相矛盾」之后，
跟着又揪出「仓位算法算不通」，顺带把**每次都长一个样**和**只有技术面没有消息面**两个短板一起补了。

---

### 一、仓位算法：从「算不通」改成「本金 = 保证金，名义 = 本金 × 杠杆」

旧实现从「单笔只亏 5U」反推名义（= 5 ÷ 止损%），再除以 10 得保证金，末尾还叠一句自相矛盾的提示：

> · 仓位算法：止损位 2,495.06（距离 1.6%）。100U 本金、单笔亏 5U → 做多名义 ≈ 310U，
>   **10x 占保证金 ≈ 34U**（**10x 保证金要占 31U**，太重，建议降到 9x 左右）

三层问题：① 一句话里两个「10x 保证金」（34U / 31U）；② **建议「降到 9x」方向是反的** ——
旧口径下保证金 = 名义 ÷ 杠杆，降杠杆只会让保证金**变大**（310÷9 ≈ 34U > 31U）；
③ 口径本身读者复算不出来。

现在按你给的口径直写：

> · 仓位算法：**100U 本金 = 100U 保证金，10x 杠杆 → 做多开单名义 ≈ 1000U**；止损位 2,495.06
>  （距离 1.6%），打止损亏 ≈ 15.7U（占本金 16%，偏重；想稳一点把杠杆降到 6x，名义 ≈ 600U、亏 ≈ 9.4U）。

偏重时给的降杠杆建议改为**按「把单笔亏损压到本金 10% 左右」反推**（`lv = 10 ÷ (100 × 止损距离%)`）——
这个口径下降杠杆会同步缩小名义，方向才对；止损 8% 时直接建议 1x（亏 8U），不再是「降到 5x 照做仍亏 40%」。

### 二、四种文章风格，每次换一换

同一份事实，四种说法（`compose(style=…)` 可锁定，不传则随机轮换，`meta.json` 记录命中的风格）：

| 风格 | 结构 |
|---|---|
| 冷静复盘体 | 事实 / 情绪面 / 观点 / 计划 分栏陈述（原版） |
| 交易员日记体 | 第一人称、一句一行、带盯盘时间感 |
| 自问自答体 | 小标题提问，一段一问一答 |
| 直给结论体 | 开头三句给判断，短句零铺垫 |

**核心约束：只换叙述，不换数字。** 点位、止损、仓位、反向剧本出自同一个 `_facts()`，
四种风格共用；回归测试逐字比对四风格的 入场 / 止损 / 反向剧本 三行完全一致，
并有 AST 护栏禁止渲染器绕开 `_facts()` 自己取数 —— 防止「换个写法把止损也换了」。

### 三、代币消息面

- 新增 `scanner.coin_news(symbol)`：PANews 中文快讯 + CoinDesk / Cointelegraph RSS（零 Key），
  与 news-sentiment 技能同源；关键词情绪统计（中英词表），某源失败只记 `failed` 不当致命错误。
- 文章里补一段消息面：相关标题条数 + 利多/利空/中性占比 + 引用两条真实标题（带来源与时间）。
  只作陈述、不覆盖 K 线/费率的硬数据；**取不到新闻时整段省略，发文绝不因此失败**。
- 1~2 个字母的 ticker 只认原文里独立的大写出现（避免 `op` 误命中 `operation` 一大片）。

### 四、文案精简（按反馈逐条删）

- 页脚删掉「封面图是 90 日 K线加成交量…」整行说明；
- 新闻句删掉「（某某源这次没抓到…）」的实现细节和「消息只是背景板…」的说教尾巴。

### 五、验证

- 新增 `tests/test_v1540_square_sizing.py` **12/12**（口径 / 距离与亏损一致 / 建议杠杆必须真降风险 /
  一句里保证金只能出现一次 / 守卫用例：把旧原文喂进判据必须判不合格 / AST 护栏）。
- 新增 `tests/test_v1540_square_styles.py` **14/14**（四风格可生成且互不相同 / 各自结构特征 /
  **四风格点位剧本逐字一致** / AST 护栏：渲染器不得重算事实 / 消息面静默降级与只引用真实标题 /
  短 ticker 防误命中）。
- v1.5.39 的护栏同步上移到 `{_article, _article_full, _facts}`（任何风格都绕不开 `_invalid_line()`）。
- 全量 **13 套件通过**；`tsc --noEmit` 退出码 0。
