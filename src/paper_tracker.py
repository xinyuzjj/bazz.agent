...
发文成功后按文章计划建纸单（100U 保证金 × 10x），本模块后台每 60s 复查一次
价格，驱动状态机：pending（挂单中）→ open（已入场）→ win（已获利）/ loss（已亏损），
7 天观察期到期强制结算（未入场 → noentry）。

**plan 节与引擎无关**（v1.6.6）：代币引擎（SMC，square_rich）与妖币引擎（剧本三轴，
square_monster）产出**同结构的 plan**，所以两条发文链路都会自动建纸单；妖币引擎在不该
进场的位阶会写 `plan: null`，那时不建单 —— 这是正确行为，不是漏单。
