...
| 版本 | 核心内容 |
|---|---|
| **v1.5.11** | portable.zip 退役，setup.exe 成为唯一全量包：① release.yml 不再打/传 portable zip（BAZZ_ZIP→BAZZ_SETUP），Release 资产 = setup.exe + delta + SHA256SUMS + MANIFEST.json；delta 基线是上一版 MANIFEST 清单对比，不依赖上一版 zip，增量更新零影响；② updater 全量回退切 setup.exe 静默安装——_pick_asset 认 *-setup.exe、_download_worker 对 exe 跳过 zip 结构校验（大小校验+apply 时 SHA256 强校验兜底）、_diff_worker 产物强制 .zip 命名（apply 按扩展名分流）、新增 _write_installer_script（等退出→杀后端→setup 挪出安装根→/VERYSILENT /DIR=当前根原地覆盖→校验新 exe→清理→重启；installer.iss 无 [InstallDelete] 且 workspace 不在 [Files] → 数据天然保留；/DIR 强制指回当前目录防解压目录装到默认路径）；单测：资产选择/delta 改名/脚本生成（PowerShell Parser 语法验证）/分流断言全过 |
| **v1.5.10** | Agent↔技能链路健壮性：① run_skill 超时 90s→180s（coin-report 全维度拉数冷缓存/慢代理下 90s 不够即超时失败）+ 输出截断 6000→12000（market-data klines 90 根 JSON ≈8KB 被截断）；② 直跑 .agents/skills/*/cli.mjs 报错改为明确指引用 run_skill（减少无效重试）；③ 审计确认：技能落盘 cwd 锚定 workspace、雷达纯 WS 快照、状态机守卫齐全 |
