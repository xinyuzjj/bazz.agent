...
- **规模**：后端 `src/` **34 个** py 模块 + `desktop_app.py`（**137 条路由**，含 1 个 `@app.websocket`）；前端 12 个视图 / 40 个文件；**21 个离线回归测试套件**（`tests/test_*.py`）+ 1 套浏览器视觉验收（`tests/ui_preview_check.py`）
...
**运维待办**：安装版仍是 1.5.61，本轮改动落地后需覆盖安装新版，否则全部不生效。
