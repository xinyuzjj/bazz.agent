# BAZZ.AGENT 项目状态交接（v1.5.37 代理池功能性缺陷修复已落地并发布 · 2026-09-12 更新）
...
- **当前版本**：**v1.5.37（代理池功能性缺陷修复：不再开机静默接管流量 / 取消连接真的回直连 / 点一下不再卡死 / 订阅文件只留 proxies 段）**；
  `package.json` 与最新 git tag 均为 v1.5.37
- **规模**：后端 `src/` 32 个 py 模块 + `desktop_app.py`（**130 条路由** = 129 个 `@app.<method>(` + 1 个 `@app.websocket`，`@app.<method>(` 计数 v1.5.34→v1.5.37 恒为 129）；前端 12 个视图 / 40 个文件；**10 个离线回归测试套件** + 1 套浏览器视觉验收（`tests/ui_preview_check.py`）
...
.venv/Scripts/python.exe tests/test_v1537_proxy_identity.py # 19/19（代理池：启动直连 / 拆栈 / 锁外等待 / provider 抽段）
...
- **v1.5.37**（代理池功能性缺陷修复）已完成并发布：`package.json` / tag 均为 v1.5.37
- **10 个离线回归套件全部通过**（用主目录自带 `.venv` 跑，它带 `requests`）：
  ...
  test_v1534_image_preview 7/7 · test_v1535_square_delete 12/12 · **test_v1537_proxy_identity 19/19** ✅
...
- **⚠️ 源码扫描断言一律走 AST**：`"xxx()" not in body` 这类纯文本切片会被**文档字符串和注释**骗到
  （v1.5.37 的 `bootstrap()` 文档正文里就写着 `_revive_kernel_async()`）→ 统一用 `ast.walk` 只看 `ast.Call`
