# BAZZ.AGENT 项目状态交接（v1.5.67 契约漂移修复 · 2026-09-16 更新）
...
- **当前版本**：**v1.5.67（四处「两侧契约漂移」修复 + 一致性回归护栏 —— 钱包 `/api/wallet/status` 404、i18n 键漂移四处）**；
  `package.json` 与最新 git tag 均为 v1.5.67
- **规模**：后端 `src/` **35 个** py 模块 + `desktop_app.py`（**143 条路由**＝142 条 HTTP 路由 + 1 个 `@app.websocket`）；前端 12 个视图 / 40 个文件；**24 个离线回归测试套件**（`tests/test_*.py`）+ 1 套浏览器视觉验收（`tests/ui_preview_check.py`）
...
## 二、近期发布版本（v1.5.12 → v1.5.66）

| 版本 | 核心内容 |
|---|---|
| **v1.5.67** | **四处「两侧契约漂移」修复 + 一致性回归护栏（用户问「还有哪里有问题」）**。共同点：都不是逻辑 bug，而是**两侧对不上** —— 跑一遍看不出来、也不报错，**只有真・切函数的 UI 会错**。① **钱包 4 通道面板的 Agent 钱包那一格一直是坏的**：管理页 raw fetch `/api/wallet/status`，而后端只有 `/api/wallet`（返回 `{cli,status,commands,daily_caps,…}`）→ 真实 app 里就是 **404** → 那一格**恒显示「未连接」**，哪怕 baw 其实扫码登录过了。**藏了几个版本的原因**：`preview/mock.ts` 里**恰好 mock 了**这条 → 隔离预览一切正常，**mock 把真 bug 盖住了**。→ **修**：新增薄路由 `GET /api/wallet/status`，只回 `get_wallet_state()["status"]`，与 `/wallet/cex/status`、`/wallet/web3/status` 对称（那条链路是 30s 轮询，不必每次带 COMMAND_TREE）。② **i18n 键漂移四处**（`locales.ts` 契约写着 zh/en 必须成对，而 `i18n.tsx` 实现是 `dict[key] ?? key`、**没有 zh 兜底** → 缺键 = 界面把 key 当文案印出来）：**22 个 `track.*` 只有 zh**（v1.5.63 把订单跟踪面板挂到交易所视图时只补了中文 → 英文界面整块显示 `track.title`…）· **`common.saved` 只有 en**（**中文**保存 toast 显示「common.saved」）· **`common.cancel` 两本都没有**（订阅授权弹窗取消按钮双语都显示裸 key）· **`markets.trackWin` 只有 en**（**中文**行情页战绩条的胜率标签显示裸 key）。→ **修**：补齐后 zh/en **各 1335 键、集合完全相等**。⚠️ 别把 `markets.trackWin` 「改成」看起来更正的 `markets.track.winRate` —— 后者是更早一版的遗留键、**前端已无任何调用方**，这次修的是把**使用中**的那个键补进 zh。③ **新增 `tests/test_v168_i18n_parity.py`**（4 组 20 断言）：键集合 zh==en 且各自**无重复键** · 前端 1164 个 `t("…")` 用到的键必须两本都有 · `api.ts` 98 个端点与非 api.ts 的裸 `fetch("/api/…")` 都必须能在后端找到路由 · **mock 反查**（预览 mock 不得伪造后端不存在的端点，这正是 ① 能藏几个版本的机制；已登记 2 条历史占位 `/api/admin`、`/api/stats`，前端无调用方）。**⚠️ 后端路由一律走 AST 提取、不做子串匹配** —— 这次新加的路由，其 **docstring 本身就在解释 `/api/wallet/status` 为什么存在**，子串匹配会让「路由存在」这条断言**永远通过**，正好把这个 bug 放过去。**已做变异验证**（不是一个改不红的测试）：四个修复逐个「退回」到修复前状态，四组检测各自都能抓到并报错，恢复后全绿。**验证**：全量 **24 套件**（原 23 + 本轮新增 1）通过、`tsc --noEmit` 退出码 0；**未触碰止损 10% / 10x 杠杆，未改任何引擎判据** |
| **v1.5.66** | **妖币「控盘度/燃料」不再每只币都一样（用户实测反馈：「控盘度和燃料为什么每一个都是显示的一样」）—— 查出来是三个真缺陷叠加**。① **数据饥饿（主因）**：雷达 v2 的确认层只覆盖**前 24 个候选**（`get_radar_v2` 里 `cands = ...[:24]`，为控 REST 调用量），而它最终输出 40+ 行 —— 实测 **55 只里 32 只没有任何确认层因子**；更糟的是雷达用 `cf.get(k, 0.0)` 兜底把它们写成**字面 0.0**，消费方分不清「真没变化」和「压根没测」。燃料轴在这些行上只能看到资金费率，于是**永远输出「中性」**。→ **修**：新增 `_ensure_factors()` 单币按需补确认层（3~4 个 REST，180s 缓存，`scanner.confirm_factors` 公开入口），补到 `oi_last` 后还必须 **`_recompute_manip()` 重算控盘指纹** —— 否则「换手畸高」（合约成交 ÷ 持仓 > 10x）分母为 0 **永远不成立**，那半池只能停在「未见控盘指纹」（雷达输出行因此补带 `fut_qv / spot_qv / no_spot / oi_usd` 四个控盘底料）。② **文案相同**：同一档位所有币共用一句**死文案**（37 只「中性」共用一个字符串），且卡片只显示档位+这句话、**看不到命中了什么**。→ **修**：两轴 `reading` 一律带**实测读数与命中项**（同档位两只币文案必然不同，如「费率 +0.005% · taker 0.40 · RVOL15 0.2x · OI15 −0.9% · 大户比 2.71」vs「费率 −0.012% · taker 1.12 · RVOL15 1.9x」）；「未见控盘指纹」改为**列出查过哪五项**；命中时给该指纹的**人话解释**；记 0 分的条目（大户拥挤）单列「**关注**」（检测到了就要看得见）。③ **死规则**：`taker_ratio >= 1.85` **自上线起一次都没触发过** —— 实测 40 个最高成交额合约 `takerlongshortRatio` min 0.439 / 中位 1.110 / p90 1.426 / **max 1.790**，`>=1.85` 命中 **0 个**；而它在四处被用（`_stage_of` 点火判据、`_radar_score` +3、依据文案、燃料「买盘主导」）。→ **修**：改 **1.30**（主动买 ≈ 主动卖 1.3 倍，实测命中 10/40 = top 25%），四处同改并落成命名常量 **`_TAKER_BUY_DOMINANT`**（monster 侧镜像 + 测试钉住）。**⚠️ 这一条会产生行为变化**（点火更易命中、评分 +3 开始可能加分），不是纯显示修复 —— 只改一处会制造口径漂移。④ **「没测」与「中性」分家**：新增档位 **「未能判定」**，确认层补不到时不报「中性」而明说「这是『没测』不是『没有燃料』」；**只有现货无合约市场**的币（如 SPCXBUSDT）单独措辞「OI/持仓额/大户比/费率**本就不存在**」，但量能这类现货口径也有的读数照常给「有油」。⑤ **验证**：新增 `tests/test_v167_monster_factors.py`（4 组 60+ 断言：阈值镜像与「源码里 1.85 必须消失」、补取命中/缓存/降级三态、`oi_usd` 重算、「没测≠中性」、**同档位两只币文案必须不同**、现货与合约口径措辞分离）；既有 `test_v166_monster_engine.py` 全过；**全量 23 套件**通过、`tsc --noEmit` 退出码 0；真机实测 55 只里 32 只走补取、`买盘主导` 命中 **0/49 → 4/49**、档位组合从高度聚集变为覆盖 7 类 |
| **v1.5.65** | **妖币剧本进度带游标修补（纯视觉，一行逻辑）**。v1.5.64 出图做视觉验收时发现叠字：进度带上那个指「当前格」的三角游标原先画在**高亮框外侧上方**，当「当前格」恰是**第 1/2 格**时游标正好压住左上角表头「妖币剧本 · 演到第几格」，把中间的「·」和后面的「演」糊成一团 —— 而**吸筹（第 1 格）正是引擎最常给出「可埋伏」的那一格**，露脸最多的图带着瑕疵。**修复**：游标改为贴在**高亮框内侧顶部**、指向框内，与表头在纵向上彻底脱开。只动 `src/square_monster.py` 的 `_draw_stage_band` 一个 polygon 坐标；封面「90d 高/低」「现价」标注、三轴卡、出场四段、正文文案全未动，代币 SMC 引擎一行未碰。**验证**：妖币套件 90+ 断言全过 + 全量 22 套件全绿 + `tsc --noEmit` 退出码 0；用真实雷达数据重渲染 EIGENUSDT（吸筹·可埋伏）与 HEIUSDT（已拉升·窗口已过）两张对照样张，放大 3 倍逐像素复核游标与表头不再相交。**踩坑**：一条命令里连跑 22 个套件会被沙箱 `safe-delete` 保护（阈值 50 次/轮）拦掉后半段，报 `SAFE_DELETE_BULK_CONFIRM_REQUIRED` —— 那是**误报不是代码挂**，逐个单独跑全绿 |
