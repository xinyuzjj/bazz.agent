# BAZZ.AGENT 项目状态交接（妖币雷达问题清单落地完毕 · v1.7.1 已发布 · 2026-09-17 更新）

> ✅ **v1.7.1 已发布**：把 `outputs/monster-radar-analysis.html` 的问题清单**全部处理完** ——
> 13 项问题 + 结构缺口 C1/C2/C4 + 档一七项（含 §16 阈值命中率监控）+ 档二四项
> （C6 本地时序库 / C5 回放框架 / #10 成本模型 / #11 标为「实测受阻」）+ D1 + **C3 延迟治理**
> 全部落地为真实代码修改。本轮（v1.7.1）另修掉 #10 的两处遗留：**资金费率结算周期被硬编码 8h**
> （实测 4h 才是多数 → 对 46% 的币低估 2 倍成本）与**滑点标定闭环断裂**
> （`ts_series.spread_bps` 存的是现货口径，纯合约币恒为 None）。**档四五项**逐条标清阻塞原因
> （3 项需外部数据源、2 项需用户拍板），**6 个未调用端点**逐条给出实测裁决（接 2 弃 4）。
> 全量 **27 套件通过** / `tsc` 干净 / `vite build` 成功 / 真实 e2e 扫描通过。
...
- **当前版本**：**v1.7.1（已发布）**；`package.json` 与最新 git tag 均为 v1.7.1。
  工作区与远端一致，无未发版改动
- **规模**：后端 `src/` **36 个** py 模块 + `desktop_app.py`（**141 条路由**＝140 条 HTTP 路由 + 1 个 `@app.websocket`，AST 实测）；前端 12 个视图 / 40 个文件；**27 个离线回归测试套件**（`tests/test_*.py`：26 个 `test_v*` + `test_paper_orders`；另有 `tests/_mutate_v170.py` / `tests/_mutate_v171.py` 变异验证脚本与 `tests/_e2e_*.py` 真实链路验证脚本，均不参与全量）+ 1 套浏览器视觉验收（`tests/ui_preview_check.py`）
...
# 离线回归测试（27 个套件；用主目录自带 .venv 跑最省事 —— 它带 requests）
# ⚠️ 一条命令里连跑全部 27 个会被沙箱 SIGTERM 掐断（实测 12 套后中断）→ 分 2~3 批跑
...
# 真实链路 e2e（联网、走本机 mihomo 7899；BAZZ_WORKSPACE 指向临时目录，不碰用户数据）
...
git -c http.proxy=http://127.0.0.1:7899 -c https.proxy=http://127.0.0.1:7899 push origin main v1.7.1
```
